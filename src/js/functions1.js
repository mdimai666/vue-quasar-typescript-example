export function array_move(arr, old_index, new_index) {
  ...
  export function downloadStringAsXML(string, exportName) {
    var json = (string);
  ...
  export function removeNonWords(st){
      var s = st.replace(/[^\wа-я]+/ig,'');
      return s;
  }
  
  export function removeNonWords2(st){
      var s = st.replace(/[^\wа-я\-\_\.]+/ig,'');
      return s;
  }
  
  export function forwardSlash(str){
   ...
   * {0: value1, 1: value 2}
   */
  export function toObjectArray (arr) {
      var rv = {};
  ...
  export function fullUrl(){
    let h = urlJoin.apply(null, arguments);
    if(h.indexOf('http')<0) h="//"+h;
    return h;
  }
  
  export function all_childs(arr,id, prop_parent_id,__maxDepth = 15){
  ...
  let date_map = {
     ...
     * @returns {String}
     */
  export function translate_monthName(val){
    return date_map[val] || val;
  }
  
  export function translate_monthNamePrimary(val){
    return date_map_0[val] || val;
  }
